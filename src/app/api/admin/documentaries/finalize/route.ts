...
import { addTrailerToDocumentary, createDocumentary, getDocumentaryById, setDocumentaryThumbnail } from '@/lib/db';
import { assertR2ObjectExists, r2ObjectUrl, type R2AssetKind, validateR2Upload } from '@/lib/r2';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.key || !body.contentType || !['documentary', 'trailer', 'thumbnail'].includes(body.kind)) {
      return NextResponse.json({ error: 'Invalid upload details' }, { status: 400 });
    }
    validateR2Upload(body.kind, body.contentType, body.size);
    const expectedPrefix = body.documentaryId ? `documentaries/${body.documentaryId}/` : 'documentaries/new-';
    if (!body.key.startsWith(expectedPrefix) || !body.key.includes(`/${body.kind}/`)) {
      return NextResponse.json({ error: 'Invalid uploaded asset' }, { status: 400 });
    }
    const object = await assertR2ObjectExists(body.key);
    if (object.ContentLength !== body.size || object.ContentType !== body.contentType) return NextResponse.json({ error: 'Uploaded file could not be verified' }, { status: 400 });
    ...
    if (body.kind === 'documentary') {
      ...
      return NextResponse.json({ success: true, documentaryId, message: 'Documentary uploaded successfully' });
    }
    ...
    if (body.kind === 'trailer') {
      ...
      return NextResponse.json({ success: true, message: 'Trailer uploaded successfully' });
    }
    await setDocumentaryThumbnail(body.documentaryId, assetUrl, body.key);
    return NextResponse.json({ success: true, message: 'Thumbnail uploaded successfully' });
  } catch (error) {
    console.error('Error finalizing video upload:', error);
    return NextResponse.json({ error: 'Unable to process the uploaded video' }, { status: 500 });
  }
}
