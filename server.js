...
// Create a short-lived browser upload capability for Vercel Blob (server-side token only).
// This avoids exposing any long-lived write token in `window`.
app.post('/api/documentaries/blob-signed-upload', express.json(), async (req, res) => {
  try {
    if (req.cookies.admin !== 'true') return res.status(401).json({ error: 'Unauthorized' });
    ...
    const token = process.env.VERCEL_BLOB_READ_WRITE_TOKEN || process.env.VERCEL_BLOB_WRITE_TOKEN;
    if (!token) {
      return res.status(500).json({ error: 'Server missing VERCEL_BLOB_READ_WRITE_TOKEN/VERCEL_BLOB_WRITE_TOKEN' });
    }
...
app.post('/api/documentaries/blob-upload-init', (req, res) => {
  res.status(501).json({
    error: 'Not used. Use /api/documentaries/blob-signed-upload instead.'
  });
});
